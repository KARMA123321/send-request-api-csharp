...
{
    public bool CompareTo(BaseModel obj, out string? errors, Comparer? comparer = null)
    {
        var type = GetType();
        
        if (obj.GetType() != type)
        {
            errors = $"Expected type: {type} but was: {obj.GetType()}";
            return false;
        }
        
        comparer ??= new Comparer();
        var isEqual = comparer.Compare(type, this, obj, out var diffs);
        ...
        {
            errorsBuilder.AppendLine(diff.ToString());
        }
